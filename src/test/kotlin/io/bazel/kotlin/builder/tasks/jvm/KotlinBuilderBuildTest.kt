...
class KotlinBuilderBuildTest {
  ...
  private fun jvmTaskExecutor(): KotlinJvmTaskExecutor {
    val fd = FileSystems.getDefault()
    var javaHome = fd.getPath(System.getProperty("java.home"))
    if (javaHome.endsWith(fd.getPath("jre"))) {
      javaHome = javaHome.parent
    }
    val toolchain =
      KotlinToolchain.createToolchain(
        ...
        File(Deps.Dep.fromLabel("//kotlin/compiler:symbol-processing-api").singleCompileJar()),
        File(Deps.Dep.fromLabel("//kotlin/compiler:symbol-processing-cmdline").singleCompileJar()),
